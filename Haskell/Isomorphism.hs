...
isoUnMaybe (mamb, mbma) = (to, from)
  where
    to a = case (mamb $ return a) of
            Just b -> b
            Nothing -> case mamb Nothing of
                         Nothing -> undefined
                         Just b  -> b
    from b = case (mbma $ return b) of
            Just a -> a
...
two = (to, from)
        where
            ...
            from (Just Nothing) = True
            from Nothing = False
            from (Just (Just x)) = absurd x  
...
plusS = 
    (to, from)
        where
            ...
            from (Just (Left a)) = Left (Just a)
            from (Just (Right b)) = Right b
...
multS = (to, from)
        where
            to (Nothing, b) = Left b
            to (Just a, b) = Right (a, b)

            from (Left b) = (Nothing, b)
            from (Right (a, b)) = (Just a, b)
